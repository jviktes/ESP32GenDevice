...
rtc_wdt_set_time(RTC_WDT_STAGE0, 200000);  // Define how long you desire to let dog wait.
  ...
  configTime(gmtOffset_sec, daylightOffset_sec, ntpServer);
    ...
    camera.aithinker();
    //camera.uxga();
    camera.qvga(); //uxga: pomale ukladani na sd kart? //svga()
    camera.highQuality();
  ...
  server.on("/take", HTTP_GET, [](AsyncWebServerRequest* request) {
      ...
      takePicture();
      AsyncWebServerResponse* response = request->beginResponse_P(200, "text/html; charset=utf-8",tempMessageFileName.c_str());
      request->send(response);
  });
    server.on("/get", HTTP_GET, [](AsyncWebServerRequest* request) {
      Serial.print("Hi! I am ESP32-CAM get picture");
          ...
          for(int i=0;i<paramsNr;i++){
              ...
              Serial.print("Param name: ");
              Serial.println(parametrName);
              Serial.print("Param value: ");
              Serial.println(parametrValue);
              Serial.println("------");
              if (parametrName=="filename") {
...
String takePicture() {
    ...
    if (!camera.capture()) {
      ...
      debug("INFO",messageDate);
                ...
                //char path[30] = "/pictures/"+String(messageDate) +".jpg";;
                ...
                //delay(1000);
                tempMessageFileName = path;
                return tempMessageFileName;
}

String uploadPhoto(String filePath) {

  String getAll;
  String getBody;
  Serial.println("uploadPhoto filePath: " + filePath);
  ...
  if (client.connect(serverName.c_str(), serverPort)) {
    ...
    String head = "--RandomNerdTutorials\r\nContent-Disposition: form-data; name=\"imageFile\"; filename=\""+filePath+".jpg\"\r\nContent-Type: image/jpeg\r\n\r\n";
    String tail = "\r\n--RandomNerdTutorials--\r\n";
